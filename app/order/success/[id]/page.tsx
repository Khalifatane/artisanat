...
const OrderDetails = async ({ params }: { params: Promise<{ id: string }> }) => {
	...
	if (order.status !== "paid") {
		try {
			const verification = await verifyPaystackTransaction(order.id);
	...
	const customer = { email: currentOrder.email };
	const shipping = null;
	...
	return (
		<div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
			...
			<div className="text-center mb-10">
				...
				<p className="text-muted-foreground mt-2">Order #{currentOrder.lookup} has been confirmed</p>
				{customer?.email && (
			...
			<div className="border border-border rounded-lg overflow-hidden">
				...
				<div className="divide-y divide-border">
					{lineItems.map((item) => (
						<OrderItem key={item.productVariant.id} item={item} />
					))}
				</div>
