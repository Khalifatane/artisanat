	...
	const [optimisticCart, dispatchCartAction] = useOptimistic(initialCart, (state, action: CartAction) => {
		...
		switch (action.type) {
			case "INCREASE":
				return {
					...
					lineItems: state.lineItems.map((item) =>
						item.productVariant.id === action.variantId
			...
			case "ADD_ITEM": {
				...
				if (existingItem) {
					const maxQuantity =
						existingItem.productVariant.stock > 0
							? existingItem.productVariant.stock
							: existingItem.quantity;
					const nextQuantity = Math.min(existingItem.quantity + action.item.quantity, maxQuantity);
					return {
						...state,
						lineItems: state.lineItems.map((item) =>
