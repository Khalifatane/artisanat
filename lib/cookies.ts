...
export async function getCartCookieJson(): Promise<null | CartCookieJson> {
	...
	try {
		const cartCookieJson = cartCookieJson_ ? (JSON.parse(cartCookieJson_) as CartCookieJson) : null;
		if (
			!cartCookieJson ||
			typeof cartCookieJson !== "object" ||
			!("id" in cartCookieJson) ||
			typeof cartCookieJson.id !== "string" ||
			!("items" in cartCookieJson) ||
			!Array.isArray(cartCookieJson.items)
		) {
			return null;
		}
